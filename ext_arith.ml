...
let of_bexpr = function
    | Eapp (("$less"|"$lesseq"|"$greater"|"$greatereq"|"$eq_$int"|"$eq_$rat") as s, [a; b], _ ) ->
...
let mk_node_neg e g s a b = (* e : ~ a {s} b *)
    Node {
        nconc = [e];
...
let mk_node_int_lt e g a b = (* e : a < b, => a <= b - 1 *)
    Node {
        nconc = [e];
...
let mk_node_int_gt e g a b = (* e : a > b, => a >= b + 1 *)
    Node {
        nconc = [e];
...
let const_node e g = (* comparison of constants *)
    ...
    assert (f = []);
    begin match s with
    | "$less" when Q.geq Q.zero c -> [mk_node_const e g]
    | "$lesseq" when Q.gt Q.zero c -> [mk_node_const e g]
    | "$greater" when Q.leq Q.zero c -> [mk_node_const e g]
