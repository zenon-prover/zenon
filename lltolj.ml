...
let rec xhypstoadd rule =
  ...
  | Rpnotp (e1, e2) -> 
    ...
    | Eapp (Evar(p, _), ts, _), Enot (Eapp (Evar(q, _), us, _), _) ->
      List.map2 
  ...
  | Rnotequal (e1, e2) -> 
    begin match e1, e2 with
    | Eapp (Evar(p, _), ts, _), Eapp (Evar(q, _), us, _) ->
      List.map2 
	(fun x y -> [enot (eapp (evar("="), [x; y]))]) ts us, 
...
and use_defs expr = 
  ...
  | Evar (v, _) when Hashtbl.mem definition_env v -> 
    ...
    body
  | Eapp (Evar(s, _), args, _) when Hashtbl.mem definition_env s ->
  ...
  | Eapp (Evar(s, _), args, _) ->
    eapp (evar(s), List.map use_defs args)
  | Enot (e, _) ->
    enot (use_defs e)
  | Eand (e1, e2, _) ->
...
let rec xcongruence_elim s x e a b =
  ...
  | Evar (v, _) when equal e x ->
    if s
    then make_node [] (Raxiom (eapp (evar("="), [a; b])))
...
let sceqprop (e1, e2, g) =
  match e1, e2 with
  | Eapp (Evar(p, _), ts, _), Eapp (Evar(q, _), us, _) when p = q ->
...
and lkuseful e proof =
  ...
  match rule with
  | SCeqsym (a, b) -> 
    equal e (eapp (evar("="), [a; b])) && not (inlist e (rm e g))
  | SCeqref (a) -> false
  | SCtrue -> false
  | SCaxiom (a) -> equal e a && not (inlist e (rm e g))
  | SCfalse -> equal e efalse && not (inlist e (rm e g))
  | SCeqprop (Eapp (Evar(p, _), ts, _) as f1, Eapp (_, us, _)) ->
    let x = equal e f1 in
    List.fold_left2 
...
let rec unify e1 e2 =
  ...
  | Eapp (Evar(x, _), args1, _), Eapp (Evar(y, _), args2, _) 
